...
const { latestDate, todaySummary, overallSummary, todayFactoryBreakdown, recentSales, recentLoads, recentExpenses, buildSummary, factories } =
...
const workerCards = computed(() => [
  ...todaySummary.value.workerPaymentByFactory
    .filter((item) => item.payoutMode === 'daily')
    .map((item) => ({
      title: `${item.factory} bugungi ishchi`,
      value: formatSom(item.paidNow),
      subtitle: 'har kun beriladi'
    })),
  ...currentMonthSummary.value.workerPaymentByFactory
    .filter((item) => item.payoutMode === 'monthly')
    .map((item) => ({
      title: `${item.factory} joriy oy oyligi`,
...
const todayFactoryCards = computed(() =>
  todayFactoryBreakdown.value.map((summary, index) => ({
    factory: factories[index] ?? `Zavod ${index + 1}`,
...
<template>
    ...
    <article class="panel overflow-hidden p-6">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="space-y-3">
          ...
          <div>
            <h2 class="page-title">{{ t('Baha uchun kunlik hisob') }}</h2>
            <p class="page-subtitle">
              {{ t("4 ta zavodning tosh kirimi, sarfi, sotuv va foydasi bir joyda ko'rinadi.") }}
            </p>
          </div>
        </div>
  ...
  <section class="space-y-3">
    <div>
      ...
      <p class="text-sm text-slate-500">{{ t("Kunlik va oylik ishchi summalari zavod kesimida alohida ko'rsatiladi.") }}</p>
    </div>
