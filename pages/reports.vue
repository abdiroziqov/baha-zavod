...
const getFactoryAdvanceExpense = (factoryName: FactoryName) =>
  Number(
    summary.value.expenseRecords
      .filter((record) => record.factory === factoryName && normalizeText(`${record.description} ${record.notes}`).includes('avans'))
      .reduce((sum, record) => sum + record.amount, 0)
      .toFixed(2)
  )
const workerReportCards = computed(() =>
  summary.value.workerPaymentByFactory.flatMap((item) => {
    if (item.payoutMode === 'daily') {
      return [
        {
          title: `${item.factory} ishchi jami`,
          amount: item.amount,
          value: formatSom(item.amount),
          subtitle: 'kunlik hisob bo`yicha'
        }
      ]
    }

    const advance = getFactoryAdvanceExpense(item.factory)
    const remaining = Math.max(0, Number((item.accrued - advance).toFixed(2)))

    return [
      {
        title: `${item.factory} yig'ilgan oylik`,
        amount: item.accrued,
        value: formatSom(item.accrued),
        subtitle: 'oy oxiriga yig`iladi'
      },
      {
        title: `${item.factory} avans`,
        amount: advance,
        value: formatSom(advance),
        subtitle: 'zavod bo`yicha avans yozuvlari'
      },
      {
        title: `${item.factory} qolgan oylik`,
        amount: remaining,
        value: formatSom(remaining),
        subtitle: 'oylikdan avans ayirilgan'
      }
    ]
  })
)
const workerReportSummaryRows = computed(() => workerReportCards.value.map((card) => ({ metric: card.title, value: Math.round(card.amount) })))
...
const getArchiveFactoryLabel = (factoryScope: string) => {
  if (!factoryScope) return ''
  return factoryScope === 'combined' ? 'Barcha zavod' : factoryScope
}
...
const productionCostRows = computed<Record<string, unknown>[]>(() => [
  ...
  { label: 'Qo‘shimcha xarajat', value: summary.value.productionComponentTotals.market },
...
const monthlyArchiveRows = computed<Record<string, unknown>[]>(() =>
  filteredMonthlyArchives.value.map((record) => ({
    ...
    factoryScope: getArchiveFactoryLabel(record.factoryScope),
...
const exportCsv = () => {
  const rows = [
    ...
    ...workerReportCards.value.map((card) => [card.title, card.amount]),
...
<template>
  ...
  <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <StatCard title="Qo‘shimcha xarajat" :value="formatSom(summary.productionComponentTotals.market)" subtitle="avtomatik ishlab chiqarish" />
  ...
  <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <StatCard
      v-for="card in workerReportCards"
      :key="card.title"
      :title="card.title"
      :value="card.value"
  ...
  {{ activeMonthlyArchive ? formatDate(activeMonthlyArchive.startDate) : '' }} -
  {{ activeMonthlyArchive ? formatDate(activeMonthlyArchive.endDate) : '' }} ·
  {{ getArchiveFactoryLabel(activeMonthlyArchive?.factoryScope ?? '') }}
</p>
    </header>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <StatCard title="Kelgan pul" :value="formatSom(activeMonthlyArchive.incomingMoneyTotal)" subtitle="oy bo`2yicha kirim" />
