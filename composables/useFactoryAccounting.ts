...
export const factories: FactoryName[] = ['Tepa shpaklevka', 'Tepa kraska', 'Past shpaklevka', 'Past kraska']
export const defaultFactory: FactoryName = 'Tepa shpaklevka'
const workerPayoutModeByFactory: Record<FactoryName, 'daily' | 'monthly'> = {
  'Tepa shpaklevka': 'monthly',
  'Tepa kraska': 'monthly',
...
export const archiveFactoryScopes: ArchiveFactoryScope[] = [...factories, 'combined']
  ...
  const clientSummaries = computed<ClientSummary[]>(() => {
    ...
    barterRecords.value.forEach((barter) => {
      const current = summaryMap.get(barter.clientName) ?? {
        ...
        lastFactory: defaultFactory as const
      }
  ...
  const resetOperationalDataKeepDebtors = (startDate: string) => {
    ...
    const preservedManualDebts = preservedDebtSummaries.map((record) =>
      normalizeManualDebtRecord({
        id: createId('debt'),
        date: effectiveStartDate,
        factory: record.lastFactory ?? defaultFactory,
